...
public class MessageUtil {
    ...
    public static void sendActionBar(@NotNull Player player, @NotNull String msg) {
        if (EngineUtils.hasPlaceholderAPI() && PlaceholderAPI.containsPlaceholders(msg))
...
//        if (PLAYER_ITEM_MESSAGE_CACHE.add(player)) {
        if (EngineUtils.hasPlaceholderAPI() && PlaceholderAPI.containsPlaceholders(message))
...
//        craftPlayer.connection.send(packet);
    ...
    public static class MessageItemEvents extends AbstractListener<NexEngine> {
        ...
        public MessageItemEvents(@NotNull NexEngine plugin) {
            super(plugin);
        }
